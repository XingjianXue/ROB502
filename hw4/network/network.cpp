...
int main(int argc, char* argv[])
{
    ...
    for (std::string const &data_filename : data_filenames)
    {
        ...
        std::ifstream ifs{data_filename};             //data_filename
        Eigen::MatrixXd X = csv2mat(ifs);

        // now call your layers
        // --- Your code here
        Eigen::MatrixXd X1 = l1.forward(X);
        Eigen::MatrixXd relu = r.forward(X1);
        Eigen::MatrixXd X2 = l2.forward(relu);
        Eigen::MatrixXd probabilities = s.forward(X2);
        // ---
