...
public:
    ...
    Table() = default;
    Table(std::vector<Correspond> tb){
    _tb.assign(tb.begin(), tb.end());       //use assign function to assign input vector to private vector
    }
    ...
    string get(const int k) const{
        ...
        for (int i = 0; i < _tb.size(); i++)
        {
            if(_tb[i].k_key == k){
                k_flag = true;
                return _tb[i].c_contant;
            }
        }
        if(k_flag == false) return " ";
    }
    ...
    std::string getcontent(int n) const{
        return _tb[n].c_contant;
    }

private:
    std::vector<Correspond> _tb;


    // --- 
};

string processSentence(const Table& t, const std::vector<int>& keySequence) {
    string ret = "";
...
int main(int argc, char* argv[]) {
    ...
    output << processSentence(filterOutOddKeys(t3), {2, 1, 3, 5, 4}) << endl;
}